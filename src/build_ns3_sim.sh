...
# build_ns3_sim.sh
#
# Compile src/ns3_wifi_sim.cc against the installed ns-3 library.
#
# Usage:
#   bash src/build_ns3_sim.sh [--debug|--release]
#
# Output:
#   src/ns3_wifi_sim  (binary placed next to this script)
#
# Requirements:
...
echo "=== NetRL ns3 WiFi simulation build ==="
echo "  Source : $SCRIPT_DIR/ns3_wifi_sim.cc"
...
if [[ -n "$NS3_PIP_PREFIX" ]]; then
    ...
    # Determine version from library filenames (e.g. libns3.44-core.so)
    NS3_VER=$(ls "$NS3_LIB"/libns3.*-core*.so 2>/dev/null | \
              grep -oP 'libns3\.\K[\d.]+' | head -1 || echo "44")
    # ns3 3.43+ requires C++20; older versions use C++17
    CXX_STD="-std=c++20"
...
g++ $CXX_STD $OPT_FLAGS \
    ...
    -L"$NS3_LIB" \
    -Wl,-rpath,"$NS3_LIB" \
    "$SCRIPT_DIR/ns3_wifi_sim.cc" \
    $LIBS \
    -o "$SCRIPT_DIR/ns3_wifi_sim"

echo "Built: $SCRIPT_DIR/ns3_wifi_sim"
echo ""
echo "Smoke test (QUIT) ..."
printf 'QUIT\n' | timeout 30 "$SCRIPT_DIR/ns3_wifi_sim" 2>/dev/null | grep -q READY && \
